...
// YwRobot 4-channel relay module (inverted signalling)
#define relay1PIN 6
#define relay2PIN 7
#define relay3PIN 8
#define relay4PIN 9
...
void loop() {
  ...
  float pHmapped = map(pHanalog,0,1023,0,14);
  ...
  */
  ...
  if (Serial.available() > 0) {
    ...
    // if it's a 'q' turn on ch1:
    if (incomingByte == 'q') {
      digitalWrite(relay1PIN, LOW);
    }
    // if it's an 'a' turn off ch1:
    if (incomingByte == 'a') {
      digitalWrite(relay1PIN, HIGH);
    }
    ...
    // if it's an 's' turn off ch2:
    if (incomingByte == 's') {
      digitalWrite(relay2PIN, HIGH);
    }

    // if it's an 'e' turn on ch3:
    if (incomingByte == 'e') {
      digitalWrite(relay3PIN, LOW);
    }
    // if it's an 'd' turn off ch3:
    if (incomingByte == 'd') {
      digitalWrite(relay3PIN, HIGH);
    }

    // if it's an 'r' turn on ch4:
    if (incomingByte == 'r') {
